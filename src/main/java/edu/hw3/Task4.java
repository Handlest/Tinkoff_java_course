...
public class Task4 {
    ...
    public static String convertToRoman(int number) {
        ...
        if (number < MIN_LIMIT || number > MAX_LIMIT){
            throw new IllegalArgumentException("Введённое число слишком " + (number < MIN_LIMIT ? "Маленькое" : "Большое"));
        }
        ...
        while (numberTemp != 0){
            ...
            for (int i = 0; i < abs(numberTemp / numbersArray.get(currentNumberIndex)); i++) {  // Не работает для отрицательных чисел
